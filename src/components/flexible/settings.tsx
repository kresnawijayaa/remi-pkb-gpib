...
export function SettingsFields({ initial }: { initial?: Settings }) {
  ...
  return <><label>Nama turnamen / tahap<input name="name" required minLength={3} maxLength={100} defaultValue={initial?.name} placeholder="Contoh: REMI PKB · Penyisihan" /></label>
    <div className="neo-fields">
      ...
      <label>Maksimal per meja<input name="capacity" type="number" min={2} max={5} required value={capacity} onChange={event => setCapacity(Number(event.target.value))} /><small>Maksimal 5 orang; meja dibagi semerata mungkin</small></label>
    ...
    {capacity > 0 && target % capacity !== 0 && <p className="neo-notice">Jumlah peserta tidak habis dibagi kapasitas. Sistem akan menambah meja dan membagi peserta semerata mungkin, misalnya 6 peserta menjadi 3–3.</p>}
  </>;
}
