...
export function PublicStandings({ name, rounds, completedTables, expectedTables, final, standings, updatedAt }: { name: string; rounds: number; completedTables: number; expectedTables: number; final: boolean; standings: PublicStandingRow[]; updatedAt: string }) {
  ...
  return <>
    ...
    {standings.length ? <>
      ...
      {visibleStandings.length ? <div className="neo-table-scroll neo-public-standing-scroll"><table className="neo-public-standing-table"><thead><tr><th>Peringkat</th><th>Peserta</th><th>Main</th><th aria-label="Jumlah peringkat pertama">P1</th><th aria-label="Jumlah peringkat kedua">P2</th><th aria-label="Jumlah peringkat ketiga">P3</th><th>Poin</th><th>Total skor</th></tr></thead><tbody>{visibleStandings.map(row => <tr key={row.number} className={row.rank <= 3 ? `is-podium is-rank-${row.rank}` : ""}><td><strong>{String(row.rank).padStart(2, "0")}</strong></td><td><strong>{row.name}</strong><small>#{row.number} · {row.community || "Tanpa komunitas"}</small></td><td>{row.played}/{rounds}</td><td>{row.firsts}</td><td>{row.seconds}</td><td>{row.thirds}</td><td><strong>{row.points}</strong></td><td>{row.totalScore}</td></tr>)}</tbody></table></div> : <section className="neo-empty-small" aria-live="polite"><strong>Peserta tidak ditemukan.</strong><p>Coba nama, nomor peserta, atau komunitas yang berbeda.</p></section>}
